...
(defn update []
	...
	(let  [ x1 (get-value :snakeHeadX)
	        ...
	        x2 (get-value :foodX)
	        y2 (get-value :foodY)]
		(if (and (and (>= y1 (- y2 10)) (<= y1 (+ y2 10))) (and (>= x1 (- x2 10)) (<= x1 (+ x2 10))))
			(update-state :snake (cons (get-value :snakeHeadX) (cons (get-value :snakeHeadY) (get-value :snake))))
			(update-state :snake (cons (get-value :snakeHeadX) (cons (get-value :snakeHeadY) (drop-last 2 (get-value :snake)))))
		)
	)	
	(redraw-canvas)
)
...
(def updates
	{:setup setup :snake update}	
)

;; Support code
(defn setup-sketch []
	(smooth)
	(println (merge (hash-map :updates updates) states))
