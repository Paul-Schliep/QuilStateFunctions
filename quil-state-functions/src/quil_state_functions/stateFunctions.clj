...
(defn setup-state [coll]
	;(println (apply set-state! (vec (interleave (map (fn [key var] key) coll) (map (fn [key var] (if (fn? var) var (ref var))) coll)))))
	(if (map? coll) (apply set-state! (vec (interleave (map (fn [key var] key) coll) (map (fn [key var] (if (fn? var) var (ref var))) coll)))))
	(apply set-state! (vec (interleave (take-nth 2 coll) (map (fn [key var] (if (fn? var) var (ref var))) coll))))
)
